    ...
    posts_to_upsert = api_posts.map do |p|
      {
        ...
        author_name: p['PostingUserDisplayName'] || p.dig('Author', 'DisplayName'),
        author_id: (p.dig('Author', 'Identifier') || p['UserId'] || p['PosterId']).to_s,
      ...
      begin
        transaction do
          api_posts.each do |p|
            ...
            post.author_id = (p.dig('Author', 'Identifier') || p['UserId']).to_s
